...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith("/chat/")) {
      debugLog("Routing to WebSocket chat handler", {
        pathname: url.pathname,
...
async function callOpenRouterAPI(messages, env) {
  ...
  // Build schema properties based on mode
  const schemaProperties = {
    responseMessage: {
      type: "string",
      description: "Response message to the user. Do not empty this field",
    },
  ...
  // Add PhoneNumber field based on mode
  if (isPhoneNumberCollectMode) {
    schemaProperties.PhoneNumber = {
      type: "string",
      description:
        "The phone number of user, if provided. If not, this field should be empt",
    };
  }
...
function validateStructuredResponse(responseContent) {
  try {
    ...
    // Validate PhoneNumber field based on mode
    if (isPhoneNumberCollectMode) {
      if (!parsed.PhoneNumber || typeof parsed.PhoneNumber !== "string") {
        throw new Error("PhoneNumber is required in phone collection mode");
      }
    }
    ...
    return {
      ...
      data: {
        ...
        PhoneNumber: parsed.PhoneNumber || "", // Include PhoneNumber in response data
      },
    };
  } catch (error) {
    return {
...
async function updateSpreadsheetAsync(
  ...
  try {
    debugLog("Starting async Spreadsheet update", {
      ...
      needsHumanSupport: structuredData.isRequestForRealPerson,
      hasPhoneNumber: !!structuredData.PhoneNumber,
      phoneCollectMode: isPhoneNumberCollectMode,
      timestamp: new Date().toISOString(),
    });
    ...
    const updatePromise = callAppsScriptPost(
      {
        ...
        needsHumanSupport: structuredData.isRequestForRealPerson,
        phoneNumber: structuredData.PhoneNumber || "", // Include phone number
      },
      env
    );
...
async function handleHumanSupportRequest(body, clientIP, env) {
  ...
  try {
    debugLog("Human support request received", {
      machineId,
      supportData:
        typeof supportData === "string" ? JSON.parse(supportData) : supportData,
    });
